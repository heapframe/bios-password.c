// this code is modified from https://github.com/dogbert/bios-pwgen/blob/master/dell.c
				  ...
				  encfuncT1 func2, encfuncT1 func3, encfuncT1 func4, encfuncT1 func5)
{
	...
	for (i = 0; i < 64; i++)
	{
		...
		{
		case 0:
			t = A + func1(func2, B, C, D, t + encblock[(i) & 15]);
			break;
		case 1:
			t = A + func1(func3, B, C, D, t + encblock[(i * 5 + 1) & 15]);
			break;
		case 2:
			t = A + func1(func4, B, C, D, t + encblock[(i * 3 + 5) & 15]);
			break;
		case 3:
			t = A + func1(func5, B, C, D, t + encblock[(i * 7) & 15]);
			break;
		}
		A = D;
		D = C;
...
char *returnts(char *message)
{
	char *ret = malloc(strlen(message) + 1);
	if (!ret)
	{
		perror("malloc failed!!!");
		return "malloc failed!!!";
	}
	strncpy(ret, message, strlen(message));
...
char *dellKeygen(const char *serial)
{
	...
	{ // hdd serial
		if (minus != NULL)
		{
			...
			return returnts("Incorrect input");
		}
		bfunc = fHDDold;
	...
	{ // service tag
		...
		btype = -1;
#ifdef allow595B
		if (strncmp(&buf1input[len1 + 1], bSuffix[t595B], 4) == 0)
			btype = t595B;
		else
#endif
		if (strncmp(&buf1input[len1 + 1], bSuffix[tD35B], 4) == 0)
			btype = tD35B;
		else
#ifdef allowA95B
		if (strncmp(&buf1input[len1 + 1], bSuffix[tA95B], 4) == 0)
			btype = tA95B;
		else
#endif
#ifdef allow2A7B
		if (strncmp(&buf1input[len1 + 1], bSuffix[t2A7B], 4) == 0)
			btype = t2A7B;
		else
#endif
#ifdef allow1D3B
		if (strncmp(&buf1input[len1 + 1], bSuffix[t1D3B], 4) == 0)
			btype = t1D3B;
		else
#endif
#ifdef allow3A5B
		if (strncmp(&buf1input[len1 + 1], bSuffix[t3A5B], 4) == 0)
			btype = t3A5B;
		else
#endif
#ifdef allow1F66
		if (strncmp(&buf1input[len1 + 1], bSuffix[t1F66], 4) == 0)
			btype = t1F66;
		else
#endif
#ifdef allow6FF1
		if (strncmp(&buf1input[len1 + 1], bSuffix[t6FF1], 4) == 0)
			btype = t6FF1;
		else
#endif
#ifdef allow1F5A
		if (strncmp(&buf1input[len1 + 1], bSuffix[t1F5A], 4) == 0)
			btype = t1F5A;
#endif
		...
		if (btype < 0)
		{
			...
			return returnts("Invalid service tag");
		}
		struct tm *time1;
	...
	if (btype == t595B && bfunc == fSVCTAG)
	{ // to check if A95B bug
		char mpw1[32];
